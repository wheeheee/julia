    ...
    if (R && ans && e->envidx < e->envsz) {
        ...
        if (vb.intvalued && lb == (jl_value_t*)jl_any_type)
            ...
            if (is_leaf_bound(lb)) {
                ...
                if (jl_is_type(lb) && lb != jl_bottom_type && vb.lb_certainty < BOUND_EGAL &&
                    ...
                    val = wrap_tvar_env((jl_value_t*)jl_new_typevar(u->var->name, lb, lb),
                                        eff_constrained || vb.lb_certainty == BOUND_EQ);
                else
                    val = lb;
            ...
            if (vb.tainted_inner)
                ...
                val = wrap_tvar_env(lb, eff_constrained);
            else if (jl_is_type(lb) && lb != jl_bottom_type && vb.lb_certainty < BOUND_EGAL &&
                     !jl_has_free_typevars(lb))
                // pinned only through equality positions, see above
                val = wrap_tvar_env((jl_value_t*)jl_new_typevar(u->var->name, lb, lb),
                                    eff_constrained || vb.lb_certainty == BOUND_EQ);
            else
                val = lb;
