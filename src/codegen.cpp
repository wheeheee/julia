...
static MDNode *best_tbaa(jl_tbaacache_t &tbaa_cache, jl_value_t *jt) {
    ...
    if (jt == (jl_value_t*)jl_datatype_type ||
        (jl_is_some_typeeq(jt) && jl_is_datatype(jl_some_typeeq_T(jt))))
...
// returns true if the call has been handled
{
    ...
    if (f == BUILTIN(is) && nargs == 2) {
        ...
        *ret = emit_ifelse(ctx, argv[1], argv[2], argv[3], rt);
        ...
        if (jl_is_some_typeeq(ty.typ) && !jl_has_free_typevars(ty.typ)) {
            jl_value_t *tp0 = jl_some_typeeq_T(ty.typ);
        ...
        if (jl_is_some_typeeq(ta.typ) && !jl_has_free_typevars(ta.typ) &&
            jl_is_some_typeeq(tb.typ) && !jl_has_free_typevars(tb.typ)) {
            int issub = jl_subtype(jl_some_typeeq_T(ta.typ), jl_some_typeeq_T(tb.typ));
        ...
        *ret = jl_cgval_t();
        ...
        if (jl_is_some_typeeq((jl_value_t*)utt) && jl_is_concrete_type(jl_some_typeeq_T((jl_value_t*)utt)))
            utt = (jl_datatype_t*)jl_typeof(jl_some_typeeq_T((jl_value_t*)utt));
        ...
        *ret = mark_julia_type(ctx, len, false, jl_long_type);
        ...
        else if (jl_is_some_typeeq(obj.typ)) {
            jl_value_t *tp0 = jl_some_typeeq_T(obj.typ);
        ...
        *ret = mark_julia_type(ctx, sz, false, jl_long_type);
        ...
        const jl_cgval_t &fld = argv[2];
        if ((jl_is_some_typeeq(typ.typ) && jl_is_concrete_type(jl_some_typeeq_T(typ.typ))) ||
        ...
        *ret = emit_isdefinedglobal(ctx, (jl_module_t*)mod.constant, (jl_sym_t*)sym.constant, allow_import, order);
        ...
        ssize_t fieldidx = -1;
        if (jl_is_some_typeeq((jl_value_t*)stt)) {
            ...
            jl_value_t *tp0 = jl_some_typeeq_T((jl_value_t*)stt);
...
static jl_cgval_t emit_expr(jl_codectx_t &ctx, jl_value_t *expr, ssize_t ssaidx_0based)
{
    ...
    if (head == jl_isdefined_sym) {
        ...
        if (jl_is_some_typeeq(ty) &&
                jl_is_datatype(jl_some_typeeq_T(ty)) &&
                jl_is_concrete_type(jl_some_typeeq_T(ty))) {
            jl_value_t *tp0 = jl_some_typeeq_T(ty);
        ...
        jl_value_t *rettype_const)
{
    ...
    for (size_t i = 0; i < nargs; i++) {
        ...
        if (is_uniquerep_Type(jt)) {
            myargs[i] = mark_julia_const(ctx, jl_some_typeeq_T(jt));
        }
...
static jl_datatype_t *compute_va_type(jl_value_t *sig, size_t nreq)
{
    ...
    for (size_t i = nreq; i < jl_nparams(sig); ++i) {
        ...
        if (is_uniquerep_Type(argType))
            argType = jl_typeof(jl_some_typeeq_T(argType));
        ...
        jl_code_instance_t *codeinst)
{
    ...
    auto get_specsig_arg = [&](jl_value_t *argType, Type *llvmArgType, bool isboxed) {
        if (type_is_ghost(llvmArgType)) { // this argument is not actually passed
            ...
            return mark_julia_const(ctx, jl_some_typeeq_T(argType));
        }
        Argument *Arg = &*AI;
        ++AI;
        AttrBuilder param(ctx.builder.getContext(), f->getAttributes().getParamAttrs(Arg->getArgNo()));
